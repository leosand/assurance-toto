...
// Usage : node wfb-flow.cjs <RELAY_SK> <CEO_PK> <SIN_PK> <CLAIM_ID>
const { finalizeEvent, getPublicKey } = require('nostr-tools/pure');
const crypto = require('crypto');
const [RELAY_SK, CEO_PK, SIN_PK, CLAIM_ID] = process.argv.slice(2);
...
async function main() {
  ...
  const appr = await post('/approvals',{correlation_id:CID,type:'claim.settlement.approve',claim_id:CLAIM_ID,montant_eur:6800,reason:'Collision vehicule/tiers — expertise externe requise',requested_by:SIN_PK});
